<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>@yield('title') - Pier.ly</title>

    {{-- icon web --}}
    <link type="image/x-icon" href="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2033%2034'%20fill-rule='evenodd'%20stroke-linejoin='round'%20stroke-miterlimit='2'%20xmlns:v='https://vecta.io/nano'%3e%3cpath%20d='M3%2027.472c0%204.409%206.18%205.552%2013.5%205.552%207.281%200%2013.5-1.103%2013.5-5.513s-6.179-5.552-13.5-5.552c-7.281%200-13.5%201.103-13.5%205.513z'%20fill='%23435ebe'%20fill-rule='nonzero'/%3e%3ccircle%20cx='16.5'%20cy='8.8'%20r='8.8'%20fill='%2341bbdd'/%3e%3c/svg%3e" rel="shortcut icon">
    <link type="image/png" href="data:image/png;base64," rel="shortcut icon">

    <link href="{{ asset('assets/dist/assets/compiled/css/app.css') }}" rel="stylesheet" crossorigin>
    <link href="{{ asset('assets/dist/assets/compiled/css/app-dark.css') }}" rel="stylesheet" crossorigin>
    <link href="{{ asset('assets/dist/assets/compiled/css/iconly.css') }}" rel="stylesheet" crossorigin>
    <link rel="stylesheet" href="{{ asset('assets/dist/assets/extensions/datatables.net-bs5/css/dataTables.bootstrap5.min.css') }}">
    <link rel="stylesheet" crossorigin href="{{ asset('assets/dist/assets/compiled/css/table-datatable-jquery.css') }}">
    <link rel="stylesheet" href="{{ asset('assets/dist/assets/extensions/sweetalert2/sweetalert2.min.css') }}">
    <link rel="stylesheet" crossorigin href="{{ asset('assets/dist/assets/compiled/css/extra-component-sweetalert.css') }}">
    @yield('styles')
</head>

<body>
    <script src="{{ asset('assets/dist/static/js/initTheme.js') }}"></script>
    <div id="app">
        <div id="sidebar">
            @include('partials.sidebar')
        </div>
        <div id="main">
            <header class="mb-3">
                <a class="burger-btn d-block d-xl-none" href="#">
                    <i class="bi bi-justify fs-3"></i>
                </a>
            </header>

            <div class="page-content">
                @yield('content')
            </div>
            @include('partials.footer')
        </div>
    </div>
    <script src="{{ asset('assets/dist/assets/compiled/js/app.js') }}"></script>
    <script src="{{ asset('assets/dist/assets/static/js/components/dark.js') }}"></script>
    <script src="{{ asset('assets/dist/assets/extensions/perfect-scrollbar/perfect-scrollbar.min.js') }}"></script>
    <script src="{{ asset('assets/dist/assets/extensions/jquery/jquery.min.js') }}"></script>
    <script src="{{ asset('assets/dist/assets/extensions/datatables.net/js/jquery.dataTables.min.js') }}"></script>
    <script src="{{ asset('assets/dist/assets/extensions/datatables.net-bs5/js/dataTables.bootstrap5.min.js') }}"></script>
    <script src="{{ asset('assets/dist/assets/extensions/sweetalert2/sweetalert2.min.js') }}"></script>
    <script>
        //untuk setiap request yang membutuhkan csrf token
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
        });
    </script>
    @yield('scripts')
</body>

</html>
